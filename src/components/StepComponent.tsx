...
export default function StepComponent({ step, onNext, onPrevious, isFirst, onStepChoice }: StepComponentProps) {
  ...
  if (step.isOptional && doStep === null) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-br from-primary to-secondary">
        <div className="bg-white/90 dark:bg-card/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 w-full max-w-md border border-white/20 dark:border-gray-700/20">
          <h2 className="text-2xl font-bold mb-4 text-center text-slate-800 dark:text-gray-100">Step {step.id}: {step.title}</h2>
          <p className="text-slate-600 dark:text-gray-300 mb-6 text-center leading-relaxed">{step.description}</p>
          {step.notes && <p className="text-sm text-slate-500 dark:text-gray-400 mb-6 italic">{step.notes}</p>}
          <div className="grid grid-cols-3 gap-3">
            <button
              ...
              className="bg-white/70 dark:bg-gray-700/50 text-slate-700 dark:text-gray-200 py-3 px-4 rounded-xl font-semibold hover:bg-white dark:hover:bg-gray-700 transition-colors border border-slate-200 dark:border-gray-600"
            >
              Skip
            </button>
            <button
  ...
  if (!step.isOptional || doStep) {
    return (
      <div className="flex flex-col min-h-screen p-4 bg-gradient-to-br from-primary to-secondary">
        <div className="bg-white/90 dark:bg-card/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 w-full max-w-md mx-auto flex-1 border border-white/20 dark:border-gray-700/20">
          <div className="mb-4">
            <div className="flex justify-between items-start mb-2">
              <h2 className="text-2xl font-bold text-slate-800 dark:text-gray-100">Step {step.id}</h2>
              <span className="text-sm bg-primary/10 text-primary px-3 py-1 rounded-full font-medium">
                {step.isOptional ? 'Optional' : 'Required'}
              </span>
            </div>
            <h3 className="text-xl font-semibold text-slate-700 dark:text-gray-200 mb-3">{step.title}</h3>
          </div>

          <div className="bg-white/60 dark:bg-gray-700/50 rounded-xl p-4 mb-6 border border-slate-100 dark:border-gray-600">
            <p className="text-slate-600 dark:text-gray-300 mb-4 leading-relaxed">{step.description}</p>

            {step.actions.length > 0 && (
              <div className="mb-4">
                <h4 className="font-semibold mb-2 text-slate-700 dark:text-gray-200 flex items-center">
                ...
                <ul className="space-y-2 ml-6">
                  {step.actions.map((action, index) => (
                    <li key={index} className="text-slate-600 dark:text-gray-300 flex items-start">
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-br from-primary to-secondary">
      <div className="bg-white/90 dark:bg-card/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 w-full max-w-md border border-white/20 dark:border-gray-700/20 text-center">
        <div className="mb-4">
          ...
          <h2 className="text-2xl font-bold mb-2 text-slate-800 dark:text-gray-100">Step {step.id} Skipped</h2>
          <p className="text-slate-600 dark:text-gray-300 mb-6">You chose to skip this optional step.</p>
        </div>
        <button
          onClick={onNext}
